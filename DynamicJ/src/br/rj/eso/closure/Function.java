...
class Function {
	...
	protected Function(Object context, Method method,FunctionType type, Object... args) {
		super();
	...
	private Object[] applyParameters(Class<?>[]parameterTypes,boolean isVarArgs,Object...newArgs){
		...
		for(int i=0;i<paramLengthWithoutVarargs;i++){
			if(args[i]==null){
				if(newArgs!=null && j<newArgs.length && parameterTypes[i].isAssignableFrom(newArgs[j].getClass())){
		...
		if(varArgsType!=null){
			...
			try{
				...
				for(;j<newArgs.length;i++,j++){
					if(varArgsType.getComponentType().isAssignableFrom(newArgs[j].getClass())){
						v.add(newArgs[j]);
					}

				}
				methodArgs[paramLengthWithoutVarargs]=v.toArray((Object[])Array.newInstance(Class.forName(varArgsType.getCanonicalName().substring(0, varArgsType.getCanonicalName().length()-2)), 0));
